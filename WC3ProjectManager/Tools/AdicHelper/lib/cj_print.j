...
library_once cjsprintf initializer cj_sprintf_updatePlayers {
    ...
    define {
        ...
        cj_sprintf_artTyp_Const = "^h^=<I2S(GetHandleId($))>^pc^=<cj_sprintf_pc[GetPlayerId($)]>^p^=<cj_sprintf_p[GetPlayerId($)]>^igold^=|cffffdc00<I2S($)>|r^ilumb^=|cff00c850<I2S($)>|r^ipc^=<cj_sprintf_pc[$]>^ip^=<cj_sprintf_p[$]>^i^=<I2S($)>^r^=<R2S($)>^b^=<cj_sprintf_b2s($)>^s^=<$>^v^=$"
